...
import com.shoppingMall.reggie.dto.OrderDto;
...
import com.shoppingMall.reggie.service.OrderDetailService;
import com.shoppingMall.reggie.service.OrderService;
...
public class OrderController {
    ...
    public R<String> submit2(@RequestBody Orders orders){
        ...
        if(user.getScores()>orders.getAmount().toBigInteger().intValue()){
            return R.error("积分不足");
        }
        orderService.submit2(orders);
        return R.success("积分消费成功");
    }

//    减少该用户积分
    @GetMapping("/sub")
    public R<String> subScore(String score){
        log.info("本次需要支付的积分为{}",score);
        String msg = orderService.subscore(Integer.parseInt(score));
    ...
    public R<String> againSubmit(@RequestBody Map<String,String> map){
        ...
        List<ShoppingCart> shoppingCartList = orderDetailList.stream().map((item) -> {
            ...
            shoppingCart.setProductParam(item.getProductParam());
